(ns solidworks.main
  "Kotodama WASM entrypoint for the Solidworks clean-room actor (L4) — Clojure port.
  ...
  facts (`solidworks.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "solidworks")
(def tier "L4")
...
(def entity-specs
  [{:entity "Part"        :plural "parts"        :id-prefix "solidwor_par"
    :fields [:partNumber :name :revision :material] :required [:partNumber :name]
    :coerce {}                       :refs {}}
   {:entity "BOM"         :plural "boms"         :id-prefix "solidwor_bom"
    :fields [:parentPartId :childPartId :quantity] :required [:quantity]
    :coerce {:quantity :int}         :refs {}}
   {:entity "ChangeOrder" :plural "changeorders" :id-prefix "solidwor_cha"
    :fields [:partId :number :status]             :required [:number :status]
    :coerce {}                       :refs {:partId "Part"}}
   {:entity "WorkOrder"   :plural "workorders"   :id-prefix "solidwor_wor"
    :fields [:partId :quantity :status]           :required [:quantity :status]
    :coerce {:quantity :int}         :refs {:partId "Part"}}
   {:entity "Robot"       :plural "robots"       :id-prefix "solidwor_rob"
    :fields [:model :cellId :status :payloadKg]  :required [:model :status]
    :coerce {:payloadKg :float}      :refs {}}
   {:entity "Program"     :plural "programs"     :id-prefix "solidwor_pro"
    :fields [:robotId :name :cycleTimeMs]        :required [:name :cycleTimeMs]
    :coerce {:cycleTimeMs :int}      :refs {:robotId "Robot"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"solidworks.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "solidworks-compat" :tier tier :entities entities} 200])
...
(defn start! [] :solidworks-compat/ready)
